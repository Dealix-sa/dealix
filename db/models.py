...
# ── Operating System Models (Phase 4) ────────────────────────
# These models support the Dealix OS layers: Revenue Command Room,
# Company Brain, Client Delivery, Outbound Safety, and Proof Packs.

class ProspectRecord(Base):
    """Prospect = a company being evaluated for outreach (ICP scoring)."""
    ...
    message_stage: Mapped[str] = mapped_column(String(32), default="not_started")
    next_action: Mapped[str | None] = mapped_column(String(255), nullable=True)
    owner_decision: Mapped[str] = mapped_column(String(16), default="hold")
    created_at: Mapped[datetime] = mapped_column(default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(default=utcnow, onupdate=utcnow)


class OutreachDraftRecord(Base):
    """Draft outreach message generated by AI, pending human approval."""

    __tablename__ = "outreach_drafts"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    prospect_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("prospects.id"), nullable=True, index=True)
    channel: Mapped[str] = mapped_column(String(32), index=True)  # email/whatsapp/sms/linkedin
    subject: Mapped[str | None] = mapped_column(String(500), nullable=True)
    body: Mapped[str] = mapped_column(Text)
    tone: Mapped[str] = mapped_column(String(32), default="professional")
    language: Mapped[str] = mapped_column(String(4), default="ar")
    status: Mapped[str] = mapped_column(String(32), default="draft", index=True)  # draft/approved/rejected/sent
...
class OutboundMessageRecord(Base):
    """Outbound message — queued, sent, or blocked by safety gate."""

    __tablename__ = "outbound_messages"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    draft_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("outreach_drafts.id"), nullable=True)
    channel: Mapped[str] = mapped_column(String(32), index=True)
    recipient: Mapped[str] = mapped_column(String(255))
    subject: Mapped[str | None] = mapped_column(String(500), nullable=True)
    body: Mapped[str] = mapped_column(Text)
    mode: Mapped[str] = mapped_column(String(16), default="draft_only")  # draft_only / controlled_live
    status: Mapped[str] = mapped_column(String(32), default="blocked", index=True)  # blocked/queued/sent/failed
...
class OutboundEventRecord(Base):
    """Event log for outbound messages — sent, delivered, replied, opt-out."""

    __tablename__ = "outbound_events"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    message_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("outbound_messages.id"), nullable=True, index=True)
    event_type: Mapped[str] = mapped_column(String(32), index=True)  # sent/delivered/replied/opt_out/bounced
    event_data: Mapped[dict] = mapped_column(JSON, default=dict)
    created_at: Mapped[datetime] = mapped_column(default=utcnow, index=True)


class DealsPipelineRecord(Base):
    """Deals pipeline — one row per deal stage transition."""
...
class ProposalRecord(Base):
    """Proposal — generated for a prospect/client."""
    ...
    total_sar: Mapped[float | None] = mapped_column(Float, nullable=True)
    status: Mapped[str] = mapped_column(String(32), default="draft", index=True)  # draft/sent/accepted/rejected
    sent_at: Mapped[datetime | None] = mapped_column(nullable=True)
    accepted_at: Mapped[datetime | None] = mapped_column(nullable=True)
    created_at: Mapped[datetime] = mapped_column(default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(default=utcnow, onupdate=utcnow)
...
class ClientRecord(Base):
    """Client = a company that has signed a proposal/contract."""
    ...
    sector: Mapped[str | None] = mapped_column(String(64), nullable=True, index=True)
    status: Mapped[str] = mapped_column(String(32), default="active", index=True)  # active/paused/churned
    ...
    monthly_retainer_sar: Mapped[float | None] = mapped_column(Float, nullable=True)
    created_at: Mapped[datetime] = mapped_column(default=utcnow)
    updated_at: Mapped[datetime] = mapped_column(default=utcnow, onupdate=utcnow)


class ClientProjectRecord(Base):
    """Project/delivery engagement for a client."""
    ...
    status: Mapped[str] = mapped_column(String(32), default="intake", index=True)  # intake/diagnosis/build/delivery/proof/closed
    start_date: Mapped[datetime | None] = mapped_column(nullable=True)
    target_end_date: Mapped[datetime | None] = mapped_column(nullable=True)
...
class ProofReportRecord(Base):
    """Proof report — before/after evidence for client delivery."""
    ...
    report_type: Mapped[str] = mapped_column(String(32), index=True)  # weekly/monthly/final/pilot
