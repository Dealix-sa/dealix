...
def run_sprint(
    *,
    ...
    # Post-run: auto-generate PDF proof pack asset and log availability.
    # Non-fatal — PDF renderer absence does not break the sprint.
    _auto_generate_proof_pack_pdf(run)
...
def _auto_generate_proof_pack_pdf(run: "SprintRun") -> None:
    """Auto-generate and cache the Proof Pack PDF after sprint completion.

    Non-fatal: any exception is logged but does not affect the sprint
    result. Stores generated bytes in run.proof_pack['_pdf_available']
    as a signal that the PDF can be rendered on demand.
    """
    try:
        if not isinstance(run.proof_pack, dict):
            return
        from auto_client_acquisition.proof_architecture_os.proof_pack_render import (
            proof_pack_to_pdf,
        )
        pdf = proof_pack_to_pdf(run.proof_pack, customer_handle=run.customer_id)
        if pdf is not None:
            run.proof_pack["_pdf_available"] = True
            run.proof_pack["_pdf_size_bytes"] = len(pdf)
    except Exception:
