...
ANGLE_MAP: dict[str, str] = {
    "real_estate_developer": (
        "كل lead عقاري بدون متابعة = احتمال خسارة العميل لمنافس. Dealix يعرض حالة كل lead ويجهّز مسودة الرد بالعربي الخليجي، "
        "مع الميزانية + الموقع + الموعد، في طابور موافقة — مسودات فقط، أنتم توافقون على كل إرسال."
    ),
    "construction": (
    ...
    "hospitality": (
        "حجوزات MICE + إفطار/سحور + قاعات = leads عربية تحتاج متابعة منظّمة. Dealix يعرض حالة كل lead ويجهّز مسودة الرد بالعربي للموافقة."
    ),
    "events": (
        "كل lead لقاعة حفل = موسم. Dealix يعرض حالة كل lead ويجهّز مسودة الرد مع التاريخ + العدد + الباقة في طابور موافقة — أنتم توافقون على كل إرسال."
    ),
    "logistics": (
    ...
    "restaurant": (
        "Dealix يعرض حالة استفسارات التموين + الحجوزات + الفرنشايز ويجهّز مسودة الرد بالعربي للموافقة، ويفرز الجاد منها للإدارة."
    ),
    "saas": (
        "Dealix رادار عمليات محكوم بالعربي الخليجي يتكامل مع HubSpot/Salesforce/Zoho. "
        "إذا تبيعون SaaS داخل السعودية، يعرض حالة inbound leads ويجهّز مسودة الرد للموافقة — أنتم توافقون على كل إرسال."
    ),
    "marketing_agency": (
        "Dealix رادار عمليات محكوم بالعربي يتكامل مع HubSpot/Salesforce/Zoho. كشركة تسويق سعودية، لكم خياران: "
...
def angle_for(sector: str | None) -> str:
    if not sector:
        return "Dealix يعرض حالة inbound leads ويجهّز مسودة الرد بالعربي الخليجي للموافقة."
    return ANGLE_MAP.get(sector.lower(),
                        "Dealix يعرض حالة inbound leads ويجهّز مسودة الرد بالعربي الخليجي للموافقة.")
