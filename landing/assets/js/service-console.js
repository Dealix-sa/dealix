...
(function () {
  ...
  function init() {
    ...
    // Track B1: try live API first (fresh YAML-derived), fall back to
    // static JSON. The static JSON is auto-regenerated by pre-commit
    // hook so it should match — but the live endpoint is the truth.
    var liveCtl = (typeof AbortController !== "undefined") ? new AbortController() : null;
    var liveTimer = liveCtl ? setTimeout(function () { liveCtl.abort(); }, 4000) : null;
    ...
    function loadLive() {
      ...
      if (liveCtl) opts.signal = liveCtl.signal;
      return fetch(LIVE_API_URL, opts)
        .then(function (r) {
    ...
    function render(payload, source) {
      ...
      if (lu && payload.last_updated) {
        lu.textContent = payload.last_updated +
          (source === "live" ? " · live" : " · cached");
      }
    }

    loadLive()
      .then(function (payload) { render(payload, "live"); })
      .catch(function () {
        loadStatic()
          .then(function (payload) { render(payload, "static"); })
