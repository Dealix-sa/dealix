...
(function () {
  ...
  // ====================================================================
  // LOAD READINESS STATS — live from /assets/data/service-readiness.json,
  // fallback to known summary if fetch fails (offline/static deploy).
  // The JSON is regenerated by scripts/export_service_readiness_json.py
  // on every YAML change, so this stays auto-honest.
  // ====================================================================
  // Internal registry status `live` means repository gates passed. Public copy
  // labels this "code-ready" because provider/customer activation is separate.
  // Counts must match landing/assets/data/service-readiness.json so the fallback
  // path tells the truth even before the async fetch lands.
  const FALLBACK_STATS = { total: 32, live: 8, partial: 0, pilot: 0, target: 24 };
  const setNum = (id, n) => { const el = document.getElementById(id); if (el) el.textContent = String(n); };

  function applyStats(stats) {
    setNum('stat-live', stats.live || 0);
    setNum('stat-partial', (stats.partial || 0) + (stats.pilot || 0));
    setNum('stat-target', stats.target || 0);
  }

  // Render fallback synchronously so the bar never shows blank.
  applyStats(FALLBACK_STATS);

  // Async upgrade to live counts from the JSON. Fetch is non-blocking;
  // if it fails (404, CORS, offline static), the fallback above stays.
  fetch('/assets/data/service-readiness.json', { cache: 'no-cache' })
    .then(r => (r && r.ok) ? r.json() : null)
    .then(data => {
      if (data && data.counts) applyStats(data.counts);
    })
    .catch(() => { /* keep fallback — never break the page */ });
