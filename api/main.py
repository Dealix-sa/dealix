...
# Dealix Ultimate Operating Layer — Founder Console internal API (defensive).
# Loaded last so a YAML / import problem cannot prevent the rest of the app
# from booting. The router self-prefixes /api/v1/internal.
try:
    from api.routers.internal import founder_console as founder_console_internal_router
except Exception as _exc:  # noqa: BLE001
    founder_console_internal_router = None
    _OPTIONAL_ROUTER_ERRORS["founder_console_internal"] = repr(_exc)
...
def create_app() -> FastAPI:
    ...
    # Dealix Ultimate Operating Layer — Founder Console internal API
    # (/api/v1/internal/*). Gated by DEALIX_INTERNAL_TOKEN in production.
    if founder_console_internal_router is not None:
        app.include_router(founder_console_internal_router.router)
