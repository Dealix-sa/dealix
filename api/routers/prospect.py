...
async def inbound_handle(body: dict[str, Any] = Body(...)) -> dict[str, Any]:
    ...
    # Build response
    _cal_url = "https://calendly.com/sami-assiri11/dealix-demo"
    responses = {
        "opt_out":              "تمام، تم إيقاف الرسائل. شكراً لوقتك.",
        "interested":           f"هلا! شكراً على اهتمامك. خلني أحجز معك 20 دقيقة demo بدون أي التزام — تقدر تختار موعدك هنا: {_cal_url}",
        "wants_demo":           f"ممتاز، نسوي demo. 20 دقيقة، اختار موعد: {_cal_url}",
        "price":                f"Starter 999/شهر، Growth 2,999، Scale 7,999. في pilot بريال × 7 أيام بدون التزام. 20 دقيقة demo أفصّل الباقة المناسبة: {_cal_url}",
        "send_details":         f"تفاصيل سريعة: Dealix = AI sales rep بالعربي الخليجي، يرد على leads خلال 45 ثانية، يؤهّل، ويحجز demos. الأفضل نشوفه معاً في 20 دقيقة على سيناريو شركتكم: {_cal_url}\nأو تصفح: https://dealix.me",
        ...
        "already_has_crm":      "Dealix ما يستبدل CRM — يشتغل كطبقة أولى فوقه. يرد بالعربي، يؤهّل، ويسلّم الـ CRM قائمة leads جاهزة. تكامل مباشر HubSpot/Salesforce/Zoho/webhook. 20 دقيقة demo: " + _cal_url,
        "arabic_concern":       f"نقطة مهمة. Dealix خليجي حقيقي، ما يكتب 'حضرتك' و'تعطفكم'. 20 دقيقة demo تختبره بنفسك على سيناريو شركتكم: {_cal_url}",
        "privacy_concern":      f"مصمم PDPL-compliant: بياناتكم في سيرفرات السعودية، opt-out في كل email، audit log كامل. 20 دقيقة نناقش compliance + demo: {_cal_url}",
