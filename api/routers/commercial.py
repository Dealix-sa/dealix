...
@router.get("/growth-cards/latest")
async def growth_cards_latest(
    _: None = Depends(_require_admin),
) -> dict[str, Any]:
    """Latest daily WhatsApp growth cards (draft-only, founder approval required).

    Generated by scripts/commercial/generate_growth_sales_cards.py, which
    dealix_command_day.sh runs daily. Returns an empty card list when the
    generator has not run yet.
    """
    report = Path("reports/commercial/growth_cards/latest.json")
    if not report.exists():
        return {
            "generated_at": None,
            "cards": [],
            "note": "no cards yet — run scripts/commercial/generate_growth_sales_cards.py",
        }
    try:
        data = json.loads(report.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise HTTPException(status_code=500, detail=f"growth cards report unreadable: {exc}") from exc
    return data
