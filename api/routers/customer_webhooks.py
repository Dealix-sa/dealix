...
# NOTE: ``/_supported-events`` MUST be declared BEFORE the ``/{handle}``
# routes below. FastAPI matches routes in registration order, so if the
# parameterized route is registered first, ``_supported-events`` would be
# captured as a handle value and fail the handle regex with 422.
