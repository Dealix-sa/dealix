...
def _build_message_template(account: dict[str, Any], score: dict[str, Any]) -> str:
    ...
    if priority == "P0":
        opening = (
            ...
            "Dealix يخدم نفس القطاع كرادار عمليات محكوم يعرض حالة كل عميل محتمل ويجهّز مسودة الرسالة التالية بالعربي الخليجي مع التزام PDPL — مسودات فقط، أنت توافق على كل إرسال. "
            "تناسبكم 20 دقيقة هذا الأسبوع نوضح كيف يطبق على وضعكم؟"
        )
    elif priority == "P1":
        opening = (
            f"مرحباً، Dealix رادار عمليات محكوم بالعربي الخليجي يخدم شركات {sector} في {city}. "
            "نعرض حالة كل عميل محتمل بعد وصوله ونجهّز مسودة الرسالة التالية في طابور موافقة — مسودات فقط، أنت توافق على كل إرسال. "
            "هل عندكم تحدي حالي مع متابعة leads؟"
        )
    else:
        opening = (
            f"السلام عليكم {name}، نقدم رادار عمليات محكوم للمتابعة بالعربي للسوق السعودي. "
            "رغبت أعرف هل تواجهون تحدي مع متابعة العملاء الجدد بعد التواصل الأولي؟"
        )
