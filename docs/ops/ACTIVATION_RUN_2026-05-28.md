...
- **B2 — `pip install -e .` was not runnable.** `pyproject.toml [project.dependencies]` was missing **24 runtime packages** that `requirements.txt` had (`python-jose`, `passlib`, `bcrypt`, `pyotp`, `qrcode`, `slowapi`, `jinja2`, `pyyaml`, `numpy`, `psycopg`, `arq`, `sentry-sdk`, the OpenTelemetry set, `cryptography`, + transitive security floors). Added them → pyproject now declares **55** deps, so the documented dev setup yields a runnable app. (`ummalqura` intentionally left optional — code guards it with a Hijri fallback.)
