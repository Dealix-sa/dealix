...
**بعد dogfooding:**
```
فرصة جديدة تدخل قائمة Dealix →
Dealix يرتّبها بالجدوى × قيمة الإيراد →
Dealix يصيغ مسودة تأهيل ومتابعة (نفس النظام الذي يستخدمه العملاء) →
Sami يراجع ويعتمد كل رسالة قبل إرسالها يدوياً →
Sami يحضر الاجتماع جاهزاً تماماً
```

**النتيجة:**
- وقت Sami يتركّز على الفرص الأنضج
- ترتيب فرص واضح بدل قائمة عشوائية
- كل قرار تواصل موثّق في سجل الحوكمة
- لا إرسال أو حجز تلقائي — القرار دائماً عند المؤسس

### المرحلة 2: Self-Nurture (Dealix يتابع عملاء Dealix)

**Dealix يصيغ مسودات:**
- Welcome message للعملاء الجدد
- Feature updates شهرياً
- رسائل احتفاظ للعملاء في خطر churn
- Referral asks بعد 90 يوم

**كل المسودات تظهر للمؤسس، والمؤسس يراجع ويعتمد قبل أي إرسال — لا إرسال تلقائي.**
...
المسار ثابت وواضح لكل العملاء:
- **Mini Diagnostic** — مجاني، الخطوة 0، لا التزام
- **7-Day Revenue Proof Sprint** — 499 ريال، الخطوة 1
- **النطاق الموسّع** — يُحدَّد بعد حزمة إثبات معتمدة

**ملاحظة:** "1 ريال" هو اختبار دفع داخلي end-to-end فقط — ليس سعراً معروضاً للعملاء.
