...
عملية إيراد مُحوكَمة مبنية للسوق السعودي:
- تحلّل قائمة فرصك وترتّبها بالجدوى × قيمة الإيراد
- تصيغ مسودات تأهيل بخليجي طبيعي (مش MSA، مش ترجمة)
- تقترح خطة متابعة والقناة الأنسب لكل فرصة
- المؤسس يراجع ويعتمد كل رسالة قبل إرسالها — لا إرسال تلقائي
- توثيق كامل: سجل قرارات حوكمة + حزمة إثبات
...
- **تشخيص مجاني (Mini Diagnostic)** — لقائمة فرصك، لا التزام
- وإن ناسبك: **سبرنت إثبات إيراد 7 أيام بـ 499 ريال**
- Dealix يرتّب الفرص ويصيغ المسودات، وأنت تعتمد كل رسالة قبل الإرسال
- تقرير يومي قصير بالمخرجات + حزمة إثبات في النهاية

محدود: 5 شركات هذا الشهر فقط.
...
السعودي الطبيعي يرد:
> "هلا [الاسم]، نبدأ بتشخيص مجاني لقائمة فرصكم — وبعده، إن ناسبكم، سبرنت 7 أيام بـ 499 ريال. أبي أفهم شركتكم أفضل — 20 دقيقة؟"
...
## شكل المخرجات (نموذج توضيحي افتراضي)

Dealix في مرحلة مبكرة. ما يلي **نموذج توضيحي افتراضي** (Hypothetical /
case-safe) يوضّح شكل المخرجات في سبرنت إثبات إيراد 7 أيام — وليس عميلاً
حقيقياً ولا نتائج مضمونة:
- قائمة فرص مرتّبة (A/B/C/D) بالجدوى × قيمة الإيراد
- مسودات تأهيل ومتابعة جاهزة لمراجعة الفريق واعتماده
- خطة متابعة 7 أيام لكل فرصة عالية القيمة
- سجل قرارات حوكمة + حزمة إثبات

النتائج التقديرية ليست نتائج مضمونة. أي case study تُنشَر لاحقاً ستكون
بموافقة العميل صراحةً، أو كنموذج آمن مُعلَّن.
...
**Dealix:**
- يقدم المنتج الفعلي (عمليات إيراد مُحوكَمة — draft-and-approve)
- يدرّب فريقكم على onboarding العميل
- يدعمكم في أول 2 عملاء (معي شخصياً)
- يدير الـ infrastructure التقنية

**العميل:**
- يحصل على طبقة عمليات إيراد مُحوكَمة فوق CRM موجود
- المسودات تظهر للعميل، والعميل يعتمد كل إجراء قبل الإرسال

---

## نموذج العائد للشريك

> الأرقام أدناه نموذج تخطيطي توضيحي، وليست نتائج مضمونة.

كل عميل يبدأ بتشخيص مجاني ثم سبرنت إثبات إيراد 7 أيام بـ 499 ريال،
ثم نطاق موسّع يُسعَّر بعد حزمة إثبات معتمدة. نموذج عمولة واضح لشركاء
التنفيذ على كل خطوة. 3-10 عملاء = مصدر دخل مستقر لوكالتك.
