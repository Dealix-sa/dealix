...
class TestAdapterIntegration:
    ...
    def test_all_adapters_produce_frozen_entities(self) -> None:
        ...
        entities = [
            ...
            normalize_learning_event(
                SimpleNamespace(
                    kind="lead_scored", customer_handle="frozen-co",
                    timestamp=datetime(2026, 7, 1, tzinfo=UTC),
                    sector="tech", channel="email", offer="pilot",
                    succeeded=True, notes_en="test", notes_ar="",
                ),
                tenant_id=TENANT_ID,
            ),
            normalize_win_loss(
                SimpleNamespace(
                    outcome="won", company="Frozen Corp",
                    lesson="Fast close", next_change="Scale it",
                    sector="saas", channel="whatsapp", offer="pilot",
                    reason="value", objection="none",
