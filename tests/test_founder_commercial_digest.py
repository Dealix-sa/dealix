...
def test_scope_requested_within_days():
    ...
    # Pin the reference date so the rolling window stays deterministic over time.
    on_date = date(2026, 5, 17)
    assert scope_requested_within_days(14, rows, on_date=on_date) is True
    assert scope_requested_within_days(3, rows, on_date=on_date) is False
