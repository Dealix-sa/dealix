...
def _make_registry(channel: ChannelKind, status: ConsentStatus):
    ...
    registry = MagicMock()
    # contactability_check calls reg.status_for(contact_id, channel) → (status, record)
    registry.status_for.return_value = (status, None)
    return registry
...
class TestWhatsAppTemplateSuppression:
    ...
    def test_active_consent_is_safe(self):
        # ConsentStatus.GRANTED is the "active consent" state per PDPL schema
    ...
    def test_revoked_consent_is_blocked(self):
        # ConsentStatus.WITHDRAWN is the "revoked/withdrawn consent" state
    ...
    def test_expired_consent_is_blocked(self):
        # No EXPIRED state in schema — expired consent maps to WITHDRAWN (also BLOCKED)
        registry = _make_registry(ChannelKind.WHATSAPP_TEMPLATE, ConsentStatus.WITHDRAWN)
        result = contactability_check(
            contact_id="c004",
            channel=ChannelKind.WHATSAPP_TEMPLATE,
...
class TestResultNotes:
    ...
    def test_safe_result_has_notes(self):
        ...
        assert result.safety_notes  # ContactabilityResult uses safety_notes field
