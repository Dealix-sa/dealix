...
async def test_router_uses_primary_provider(mock_settings):
    ...
    response = await router.run(task=Task.SUMMARY, messages=messages)
...
async def test_router_raises_when_all_providers_fail(mock_settings):
    ...
    with pytest.raises((RuntimeError, Exception)):
        await router.run(task=Task.SUMMARY, messages=messages)
...
async def test_router_increments_fallback_counter(mock_settings):
    ...
    await router.run(task=Task.SUMMARY, messages=[Message(role="user", content="x")])
...
async def test_router_usage_records_are_updated(mock_settings):
    ...
    await router.run(task=Task.SUMMARY, messages=[Message(role="user", content="test")])
...
async def test_router_concurrent_safety(mock_settings):
    ...
    tasks = [router.run(task=Task.SUMMARY, messages=messages) for _ in range(10)]
