...
def test_health_zero_signals_is_critical():
    ...
    assert h.bucket in {"critical", "blocked"}, f"expected critical/blocked, got {h.bucket}"
    assert h.overall <= 40
...
def test_health_strong_signals_is_healthy():
    ...
    assert h.bucket in {"healthy", "expansion_ready"}, f"expected healthy/expansion_ready, got {h.bucket}"
    assert h.overall >= 75
