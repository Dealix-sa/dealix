...
def test_health_zero_signals_is_critical():
    ...
    # Extended bucket system: no signals → overall < 20 → "blocked" band
    assert h.bucket in ("blocked", "critical")
    assert h.overall <= 40
...
def test_health_strong_signals_is_healthy():
    ...
    # Extended bucket system: strong signals → overall ≥ 90 → "expansion_ready"
    assert h.bucket in ("expansion_ready", "healthy")
    assert h.overall >= 75
