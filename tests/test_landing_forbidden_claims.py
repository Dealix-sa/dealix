...
ALLOWLIST: dict[str, dict[str, str]] = {
    ...
    # Diagnostic intake — "صفر cold outreach" promise + footer disclaimer
    # "Estimated outcomes are not guaranteed outcomes / النتائج التقديرية ليست
    # نتائج مضمونة". Pure NEGATION / disclaimer.
    "diagnostic.html": {
        "cold": "NEGATION",
        "guaranteed": "NEGATION",
        "مضمون": "NEGATION",
    ...
    # Copy explicitly states what Dealix DOES NOT do: "لا scraping" plus the
    # footer disclaimer "Estimated outcomes are not guaranteed outcomes /
    # النتائج التقديرية ليست نتائج مضمونة". Pure NEGATION / disclaimer.
    "trust-center.html": {
        "scraping": "NEGATION",
        "guaranteed": "NEGATION",
        "مضمون": "NEGATION",
    ...
    # ── Wave: #559 Company OS landing surfaces ──────────────────────────
    # The following pages all carry the mandated bilingual footer disclaimer
    # "Estimated outcomes are not guaranteed outcomes / النتائج التقديرية ليست
    # نتائج مضمونة" (and, where noted, the anti-cold / anti-scraping promise).
    # Every token below was verified to appear in a NEGATION / disclaimer
    # context only — never as a positive claim.
    "architecture.html": {
        "guaranteed": "NEGATION",
        "مضمون": "NEGATION",
    },
    ...
    # "7-day money-back guarantee … refund كامل" — a refund guarantee, not a
    # sales-outcome guarantee. Mirrors roi.html's refund wording; tracked for
    # founder review (keep / qualify / rephrase).
    ...
    # Data-to-Revenue Pack — "صفر cold WhatsApp / LinkedIn", "لا scraping. لا
    # إرسال آلي" + the not-guaranteed disclaimer. Pure NEGATION throughout.
    "data-pack.html": {
        "cold": "NEGATION",
        "guaranteed": "NEGATION",
        "scraping": "NEGATION",
    ...
    # Roadmap — "LinkedIn scraping automation — policy-blocked in code". The
    # term describes a blocked capability. Pure NEGATION.
    "roadmap.html": {
        "scraping": "NEGATION",
    },
