...
ALLOWLIST: dict[str, dict[str, str]] = {
    ...
    # bilingual "Estimated outcomes are not guaranteed outcomes / النتائج
    # التقديرية ليست نتائج مضمونة" disclaimer. Pure NEGATION.
    "diagnostic.html": {
        "cold": "NEGATION",
        "guaranteed": "NEGATION",
        "مضمون": "NEGATION",
    ...
    # carries the standard "not guaranteed outcomes" disclaimer. The earlier
    # "نضمن" entry was dropped — the phrase is no longer on the page.
    "trust-center.html": {
        "scraping": "NEGATION",
        "guaranteed": "NEGATION",
        "مضمون": "NEGATION",
    ...
    # --- Pages carrying only the standard bilingual disclaimer ----------
    # Each of the following pages contains exactly one phrase:
    # "Estimated outcomes are not guaranteed outcomes / النتائج التقديرية
    # ليست نتائج مضمونة" — the doctrine-mandated disclaimer. The tokens
    # "guaranteed" and "مضمون" both appear inside that negation. Pure
    # NEGATION; verified token-by-token against each page.
    "architecture.html": {
        "guaranteed": "NEGATION",
        "مضمون": "NEGATION",
    },
    ...
    # Data-to-Revenue Pack page — policy list states "صفر cold WhatsApp /
    # LinkedIn", "لا scraping" plus the standard disclaimer. Pure NEGATION.
    "data-pack.html": {
        "cold": "NEGATION",
        "guaranteed": "NEGATION",
        "scraping": "NEGATION",
    ...
    # Public roadmap — a NO-row reads "LinkedIn scraping automation —
    # policy-blocked in code". Describes a blocked behavior. Pure NEGATION.
    "roadmap.html": {
        "scraping": "NEGATION",
    },
    # Sample pilot case study — "Growth 2,999 SAR/شهر مع 7-day money-back
    # guarantee. لو ما شفت → refund كامل". A money-back/refund guarantee is
    # a service guarantee, not a guaranteed sales OUTCOME — permitted by
    # doctrine — but it is a positive use of the word, so it is tracked as
    # REVIEW_PENDING for founder sign-off (same handling as roi.html).
