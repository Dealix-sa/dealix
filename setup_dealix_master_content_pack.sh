...
Enforces the Dealix Document Standard on the canonical operating
documents that make up the Master Content Pack. Legacy runbooks, AR
playbooks, and tactical docs evolve under their own owners and are not
checked here.
...
jobs:
  company-os:
    ...
    steps:
      ...
      - name: Compile company OS
        run: python -m compileall -q dealix scripts
YAML
