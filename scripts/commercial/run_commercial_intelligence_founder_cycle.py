...
def build_cycle(*, run_date: date | None = None) -> dict[str, Any]:
    ...
    objective_keys = [
        f"{row.get('department')}::{row.get('metric')}" for row in objectives
    ]
    ...
    return {
        ...
        "proof_log": [
            {"path": str(SOURCE_PATH.relative_to(ROOT)), "sha256": _sha256(SOURCE_PATH)},
            {"path": str(OBJECTIVE_PATH.relative_to(ROOT)), "sha256": _sha256(OBJECTIVE_PATH)},
        ],
