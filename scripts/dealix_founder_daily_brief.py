...
try:
    ...
    # Friendly hint instead of a raw traceback when the dev hasn't
    # installed the project deps yet (pydantic, structlog, etc.).
    sys.stderr.write(
        f"FATAL: missing dependency '{exc.name}'.\n"
        "Run `pip install -e .[dev]` or `make first-setup`.\n"
    )
    sys.exit(2)
