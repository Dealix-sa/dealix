#!/usr/bin/env python3
"""Refresh the sales funnel snapshot. Safe; no external sends."""
...
def main() -> int:
    ...
    cache_path = root / "founder" / "sales_funnel.json"

    try:
        from api.internal.runtime_reader import sales_funnel_summary  # type: ignore
        os.environ["DEALIX_PRIVATE_OPS"] = str(root)
        payload = sales_funnel_summary()
    ...
    print(f"[run_sales_funnel_worker] status={status}")
