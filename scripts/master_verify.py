...
def run_layer(script: Path, private_ops: str | None) -> dict:
    if not script.exists():
        return {"verdict": "FAIL", "exit_code": 1, "summary": f"missing: {script.name}", "duration_s": 0.0}
    ...
    try:
        ...
        return {"verdict": "FAIL", "exit_code": 1, "summary": "timeout (>15min)", "duration_s": 900.0}
    ...
    return {"verdict": verdict, "exit_code": res.returncode, "summary": summary or "(no output)", "duration_s": duration}


def aggregate(layers: list[dict]) -> str:
    if any(l["exit_code"] == 1 for l in layers):
        return "FAIL"
    if any(l["exit_code"] == 2 for l in layers):
        return "PARTIAL"
    return "PASS"
...
def fmt_table(rows: list[dict]) -> str:
    ...
    for r in rows:
        lines.append(f" {icon.get(r['verdict'], '[?]'):<7} L{r['layer']} {r['name']:<24} {r['duration_s']:>5}s  {r['summary'][:60]}")
    return "\n".join(lines)
...
def main() -> int:
    ...
    selected = {int(x) for x in args.layers.split(",") if x.strip().isdigit()} if args.layers else None
    rows: list[dict] = []
    ...
    try:
        git_sha = subprocess.run(["git", "rev-parse", "HEAD"], capture_output=True, text=True, cwd=REPO).stdout.strip() or "unknown"
    except Exception:
        git_sha = "unknown"

    report = {"overall": overall, "warnings": warnings, "git_sha": git_sha, "private_ops": args.private_ops or None, "layers": rows}
