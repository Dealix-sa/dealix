...
def main() -> int:
    ...
    actions: list[str] = []
    if score < 70:
        actions.append("Raise launch readiness to >= 70 (run scripts/verify_dealix_launch_readiness.py).")
    if pending_drafts:
        actions.append(f"Review {len(pending_drafts)} outreach draft(s) in the approval queue.")
    research = by_status.get("research", [])
    if research:
        actions.append(f"Qualify {min(5, len(research))} of {len(research)} research targets (add evidence/warm intro).")
    if approved_drafts:
