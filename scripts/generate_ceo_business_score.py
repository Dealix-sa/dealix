"""Generate the CEO Business Score markdown and history row.

Reads evidence from the private ops directory and writes:
  - business_audit/ceo_business_score.md
  - business_audit/score_history.csv (appended)
"""
...
def append_history(root: Path, score: dict) -> Path:
    ...
    with path.open("a", newline="", encoding="utf-8") as f:
        ...
        if not exists:
            writer.writeheader()
        writer.writerow({key: score.get(key, "") for key in headers})
    return path


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--private-ops", required=True, help="Path to the private ops repo.")
