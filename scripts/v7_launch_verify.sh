...
SECRET_HITS=$(grep -rE 'sk_live_[A-Za-z0-9]{20,}|ghp_[A-Za-z0-9]{36}|AIza[A-Za-z0-9]{35}' --include='*.py' --include='*.md' . 2>/dev/null | grep -vE 'test_|EXAMPLE|placeholder|sk_live_unsigned' | head -1)
