...
- **Intelligence→Revenue→Delivery OS (targeting machine):** [docs/01_go_to_market/MARKET_INTELLIGENCE_OS.md](docs/01_go_to_market/MARKET_INTELLIGENCE_OS.md) — offline, deterministic pipeline (research → score → weakness → offer → draft → handoff → learning). One-shot daily brief: **`python scripts/targeting_daily_brief.py --in data/targeting/company_master.jsonl --out data/targeting/out`** · paid handoff: `python scripts/targeting_delivery_handoff.py --company "…"` · weekly: `python scripts/targeting_learning_loop.py`. Config in `data/targeting/*.yml`; governance in `docs/03_governance/`; delivery in `docs/04_delivery/`. No auto-send, no scraping, founder-approval gated. Tests: `tests/test_targeting_{scorecard,compliance_gate,delivery_handoff,pipeline}.py`
