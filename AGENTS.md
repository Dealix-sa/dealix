...
- **Founder reality check (start every session here):** `python3 scripts/founder_reality_check.py` · [FOUNDER_REALITY_CHECK_AR.md](docs/ops/FOUNDER_REALITY_CHECK_AR.md) — wired anchors + claimed-but-absent audit + evidence truth + 3 honest next actions. Prevents inventing engines/registries that never landed. Exit 0 only when Phase 0–1 gate is open.
