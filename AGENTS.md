...
- **Recurring Revenue Radar (one-off → retainer):** [RECURRING_REVENUE_RADAR_AR.md](docs/commercial/RECURRING_REVENUE_RADAR_AR.md) — daily portfolio expansion scanner ranking accounts by incremental MRR (2,999/3,999/4,999). `python scripts/run_recurring_revenue_radar.py` (offline; brief → `data/founder_briefs/`) · gate `python scripts/verify_recurring_revenue_radar.py` (`RECURRING_REVENUE_RADAR_VERIFY=PASS`) · API `POST /api/v1/recurring-revenue/radar` · doctrine: no revenue before paid · proof before upsell · draft-only.
