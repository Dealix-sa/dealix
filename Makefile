...
.PHONY: help install install-dev setup test test-unit test-integration \
        ...
        ceo-weekly ceo-strategic-verify
...
# ── Strategic CEO System v3 ────────────────────────────────────
# Read-only CEO weekly war room helpers. Surface the right files
# for the founder to fill in; nothing is sent or published.

ceo-strategic-verify: ## CEO: verify Strategic CEO System v3 docs are present
	$(PYTHON) scripts/verify_strategic_ceo_system.py

PRIVATE_OPS ?= dealix-ops-private
ceo-weekly: ceo-strategic-verify ## CEO: run weekly war room checklist (PRIVATE_OPS=...)
	@echo ""
	@echo "CEO Weekly War Room Files:"
	@echo "- $(PRIVATE_OPS)/founder/weekly_war_room.md"
	@echo "- $(PRIVATE_OPS)/founder/decision_log.md"
	@echo "- $(PRIVATE_OPS)/founder/monthly_strategy_review.md"
	@echo "- $(PRIVATE_OPS)/business_audit/company_scoreboard.csv"
	@echo ""
	@echo "Required CEO output:"
	@echo "- one Build/Fix/Kill/Defer/Continue decision"
	@echo "- one system update"
	@echo "- one next-week focus"
