...
.PHONY: help install install-dev install-observability install-security install-evals install-docs \
        ...
        lint format type-check security security-smoke clean run demo cockpit notion-sync doctor \
...
notion-sync: ## Sync Founder Command Center into Notion (write-only; --dry-run safe)
	$(PYTHON) scripts/sync_founder_command_center_to_notion.py $(if $(DRY_RUN),--dry-run)
