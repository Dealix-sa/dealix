...
.PHONY: help install install-dev setup test test-unit test-integration \
        ...
        v5-proof-pack v10-verify v10-reference ceo-verify finance
...
# ── CEO operating model + finance ──────────────────────────────
# Read-only finance and operating-model verifiers. PRIVATE_OPS points
# at a local clone of dealix-ops-private; nothing here ships externally.

ceo-verify: ## CEO: verify operating model, finance model, growth system, KPI tree, cadence
	$(PYTHON) scripts/verify_ceo_business_systems.py

finance: ## CEO: generate monthly finance review (set PRIVATE_OPS=/path/to/dealix-ops-private)
	$(PYTHON) -m dealix_cli finance --private-ops $(PRIVATE_OPS)
